...
const initNameLoaders = function(item, map) {
...
const initPaths = function(item, map) {
    if (!item.paths) {
        return;
    }
    const shortName = item.name.split('?').shift();
    const set = new Set();
    item.paths.forEach((ip) => {
        const name = ip.name;
    ...
    item.paths = Array.from(set).map((p) => {
