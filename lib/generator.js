...
const initAssets = function(stats, rows, map, o) {
    if (!stats.assets) {
        return;
    }
    const assets = {
    ...
    const subs = stats.assets.map(item => {
        ...
        if (ext === ".map") {
            a.name_color = "gray";
            a.size_color = "gray";
        } else if (ext === ".js" && o.typeMode === "component") {
...
const initChunks = function(stats, rows, map, o) {
    if (!stats.chunks) {
        return;
    }
    const chunks = {
    ...
    const subs = stats.chunks.map(item => {
        const nameId = getMapId(item.id, map);
        const c = {
            name: nameId,
            size: item.size
        };
        initSizeColor(c, o.overSizeColors);
        return c;
    });
    const totalSize = subs.reduce((prev, item) => prev + item.size, 0);
...
const getStatsData = (o, stats) => {
    const map = new Map();
    const rows = [];
    initAssets(stats, rows, map, o);
    initChunks(stats, rows, map, o);
