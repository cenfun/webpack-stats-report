...
const StatsReportGenerator = async (options = {}) => {
    ...
    const statsDataStr = LzString.compressToBase64(JSON.stringify(statsData));

    const content = `<script>${os.EOL}window.statsData='${statsDataStr}';${os.EOL}${scriptContent}${os.EOL}</script>`;
