...
const initSourceSize = async (m, o) => {
...
const initModulesSource = async (modules, o) => {

    o.info.hasMinifiedAndGzipSize = o.generateMinifiedAndGzipSize;

    if (!o.generateMinifiedAndGzipSize) {
        return;
    }

    const hasSource = modules.subs.find(item => item.source);
    if (!hasSource) {
        o.info.hasMinifiedAndGzipSize = false;
        return;
    }
    
    for (const m of modules.subs) {
        await initSourceSize(m, o);
    }
    modules.sizeMinified = modules.subs.reduce((prev, item) => prev + item.sizeMinified, 0);
    modules.sizeGzip = modules.subs.reduce((prev, item) => prev + item.sizeGzip, 0);
...
const initModules = async (stats, map, o) => {
    ...
    await initModulesSource(modules, o);
